...
const options = {
  ...
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_CANCELED]: {
    type: ALERT_DANGER,
    text: (_this) => {
      return `Ваша заявка на верификацию отклонена. <a class="d-link fw_600">Узнать причину</a>`
    }
  },
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_CONFIRM_MEETING]: {
    type: CHECK,
    text: (_this) => {
      return `Вы успешно назначили встречу <a class="d-link fw_600">Посмотреть детали</a>`
    }
  },
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_DISPUTE_FINISHED]: {
    type: ALERT_DANGER,
    text: (_this) => {
      return `Ваш диспут закрыт <a class="d-link fw_600">Узнать решение</a>`
    }
  },
  ...
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_FINISHED]: {
    type: CHECK,
    text: (_this) => {
      return `Ваш заказ готов <a class="d-link fw_600">Посмотреть детали</a>`
    }
  },
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_NEW]: {
    type: INFO,
    text: (_this) => {
      return `У вас новый заказ <a class="d-link fw_600">Посмотреть</a>`
    }
  },
  [CATEGORIES.CERTIFYING_SPECIALIST_ORDER_PAID]: {
    type: INFO,
    text: (_this) => {
      return `Вы успешно оплатили услугу <a class="d-link fw_600">Посмотреть детали</a>`
    }
  }
}
...
export default {
  ...
  mounted () {
    ...
    this._handleInit(options[this.item.category], () => {
      if (this.isCustomer) {
        this.$router.push({ name: 'certifying.customer.orders.show', params: { order_id: this.cs_order_id } })
      } else {
