...
const options = {
  [CATEGORIES.CASTING_APPROVE]: {
    type: CHECK,
    text: (_this) => {
      return `Модератор проверил и опубликовал ваше объявление <a class="d-link fw_600">“${_this.casting_name}”</a>`
    },
    click: (_this) => {
  ...
  [CATEGORIES.BOARD_CASTING_DELETE]: {
    type: ALERT_DANGER,
    text: (_this) => {
      return `Модератор удалил ваше объявление <a class="d-link fw_600">“${_this.casting_name}”</a>
  ...
  [CATEGORIES.BOARD_CASTING_FORCE_PUBLISH]: {
    type: CHECK,
    text: (_this) => {
      return `Ваше объявление <a class="d-link fw_600">“${_this.casting_name}”</a> опубликовано в ленте`
    }
  },
  [CATEGORIES.BOARD_CASTING_NEW_RESPONSE]: {
    type: INFO,
    text: (_this) => {
      return `Получен новый отклик на объявление <a class="d-link fw_600">“${_this.casting_name}”</a>`
    }
  },
  [CATEGORIES.BOARD_CASTING_REJECT]: {
    type: ALERT_WARN,
    text: (_this) => {
      return `Ваше объявление <a class="d-link fw_600">“${_this.casting_name}”</a> снято с публикации`
    }
  },
  [CATEGORIES.BOARD_CASTING_RESPONSE_VIEWED]: {
    type: EYE,
    text: (_this) => {
      return `Ваш отклик на объявление  <a class="d-link fw_600">“${_this.casting_name}”</a> был просмотрен`
    }
  },
  [CATEGORIES.CASTING_RESPONSE_NEW]: {
    type: INFO,
    text: (_this) => {
      return (_this.grouped ? `Новые (${_this.count}) отклики на кастинг` : 'Новый отклик на кастинг') + ` <a class="d-link fw_600">“${_this.casting_name}”</a>`
    }
  }
}
