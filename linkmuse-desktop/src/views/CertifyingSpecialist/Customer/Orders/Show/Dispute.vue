  ...
  <section class="ss-dispute">
    <template v-if="order">
      ...
      <div class="ss-dispute-desc" v-if="!is_dispute">
        Выберите причину конфликта и желаемый способ его решения.
      </div>
      ...
      <div class="ss-dispute-form">
        ...
        <div class="ss-dispute-form-wrap">
          ...
          <div class="action">
            ...
            <template v-else-if="is_dispute">
              <router-link :to="{ name: 'certifying.customer.orders.show', params: { order_id: order.id } }" class="lm-primary-button">
                подробнее о заказе
              </router-link>
            </template>
          </div>
...
import { getCertifiedUserCauses } from '@modules/CertifyingSpecialist/Enums/DisputeCauses'
import { getCertifiedUserSolutions } from '@modules/CertifyingSpecialist/Enums/DisputeSolutions'
...
export default {
  ...
  computed: {
    ...
    causes () {
      return getCertifiedUserCauses()
    },
    solutions () {
      return getCertifiedUserSolutions()
    }
  },
  created () {
