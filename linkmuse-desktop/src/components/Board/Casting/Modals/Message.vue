  ...
  <modal class="modal-center modal_write-all" v-model="modal.value" @hide="_handleCloseModal()" :header="false" :footer="false">
    <template v-if="(modal.success === false)">
      <div class="d-row justify_between align_center">
        <div class="heading heading_h4 mb_0">Массовая рассылка</div>
        {{ addressees }}
      </div>
...
Хорошего дня!`
    ...
    methods: {
      ...
      _handleClickSendButton () {
        ...
        this.$api.post(`api/v2/boards/castings/${castingId}/responses/messages`, { message, state, attachments }).then(response => {
          let { data } = response
