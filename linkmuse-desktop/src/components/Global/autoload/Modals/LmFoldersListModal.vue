<template>
  <modal class="modal-message modal-center modal-folders" title="Добавить в подборку" :footer="false" v-model="modal.value" @hide="_handleCloseModal">
    <div class="add-cart">
      <div class="friends--list_create_cart">
        <template v-if="(showCreateFolder === false)">
          <div @click="_handleClickOpenCreateFolder">
            <i class="icon-Add"></i>
            <span>Создать новую подборку</span>
          </div>
        </template>
        <template v-if="(showCreateFolder === true)">
          <input id="folderName" autofocus ref="folderName" name="name"
                 v-model="fields.name"
                 type="text"
                 placeholder="Введите название"
                 v-validate="'required|max:256'"
                 :class="{ 'error': { 'error': (verrors.has('name')) } }"
                 @keyup.enter="_handleClickCreateFolder">
        </template>
      ...
      <div id="container-carts" style="max-height:236px; overflow-y: auto;">
        ...
        <lm-loader v-if="(folders.loading === true)" class="pd-26"></lm-loader>
      </div>



      <template v-if="(showCreateFolder === false)">
        <a href="javascript:void(0)" class="write button" @click="_handleCloseModal">Сохранить</a>
      </template>
      <template v-else-if="(showCreateFolder === true)">
        <template v-if="(modal.loading === true)">
          <lm-loader :style="{ margin: '0 auto', marginTop: '32px', marginBottom: '32px' }"></lm-loader>
        </template>
        <template v-if="(modal.loading === false)">
          <a href="javascript:void(0)" class="write button" @click="_handleClickCreateFolder">Создать</a>
        </template>
      </template>
    </div>
  </modal>
        ...
        if (index > -1) {
          ...
          this.$api.post(`api/v1/folders/${folderId}/users`, { user: userId }).then((response) => {
            ...
            this.$notify({
              type: 'error',
              title: 'Ошибка',
              text: 'Произошла ошибка при добавлении в подборку.'
            })
        ...
        if (index > -1) {
          ...
          this.$api.delete(`api/v1/folders/${folderId}/users/${userId}`).then((response) => {
            ...
            this.$notify({
              type: 'error',
              title: 'Ошибка',
              text: 'Произошла ошибка при удалении из подборки.'
            })
