    ...
    <div class="ss-dispute-warning-head" @click="_handleToggleShowContent">
      ...
      <i class="icon-more" v-if="initiator">
        <div class="actions-item">
          <ul>
            <li>
              <router-link :to="{ name: 'certifying.executor.orders.show.dispute', params: { order_id: order_id } }">Подробнее</router-link>
            </li>
          </ul>
        </div>
      </i>
    ...
    <div class="ss-dispute-warning-content" v-if="show">
      ...
      <template v-else>
        ...
        <template v-else-if="!initiator">
          <h2>Специалист начал диспут 😱</h2>
          <div class="ss-dispute-warning-desc">
            Что-то пошло не так, и <b>специалист</b> остался вами <br> недоволен.
