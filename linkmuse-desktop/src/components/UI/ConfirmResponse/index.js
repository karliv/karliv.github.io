...
export default {
  install (Vue, options = {}) {
    ...
    Vue.prototype.$confirmResponse = (config) => {
      return new Promise((resolve, reject) => {
        ...
        confirmInstance = new ConfirmConstructor({
          ...
          data () {
            return {
              modal: false,
              cancelTxt: config.cancelTxt || options.cancelTxt || 'Отмена',
              title: config.title || options.title || 'Подтвердите действие',
              confirmTxt: config.confirmTxt || options.confirmTxt || 'Подтвердить',
              message: config.message || options.message || null
            }
          },
          mounted () {
            this.$nextTick(() => {
