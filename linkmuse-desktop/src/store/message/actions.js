export const fetchDialogs = ({ commit, state, rootState: { $api } }, { rewrite = false }) => {
  return new Promise((resolve, reject) => {
    ...
    commit('SET_DIALOGS_INITIALIZED', true)
...
export function createDialog ({ commit }, data) {
  return new Promise((resolve, reject) => {
    try {
      commit('CREATE_DIALOG', data)
      resolve && resolve()
    } catch (e) {
      reject && reject()
    }
...
export function destroyDialogById ({ commit }, id) {
  return new Promise((resolve, reject) => {
    try {
      commit('DESTROY_DIALOG_BY_ID', id)
      resolve && resolve()
    } catch (e) {
      reject && reject()
    }
