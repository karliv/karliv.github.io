...
export const fetchAllServices = ({ commit, rootState: { $api } }) => {
  ...
  return $api.get('api/v2/certifying-specialists/services', { 'include': 'attachments' }).then(response => {
...
export const clearOrderById = ({ commit, rootState: { $api } }) => {
  return new Promise((resolve, reject) => {
    commit('CLEAR_SINGLE_ORDER_DATA')
    commit('SET_SINGLE_ORDER_LOADING', false)
    commit('SET_SINGLE_ORDER_ERROR', false)
    resolve()
  })
...
export const getCountersOrdersByExecutor = ({ commit, rootState: { $api } }) => {
  ...
  return $api.get(`/api/v2/certifying-specialists/users/orders/counters`).then((response) => {
    let { data: { all, not_paid, paid, dispute, finished, canceled } } = response

    commit('SET_ORDERS_EXECUTOR_COUNTERS_DATA', { all, not_paid, paid, dispute, finished, canceled })
...
export const fetchSettingsStudios = ({ commit, rootState: { $api } }, { specialist_id, include }) => {
  commit('SET_SETTINGS_STUDIOS_LOADING', true)

  return $api.get(`/api/v2/certifying-specialists/studios`, { include }).then((response) => {
    let { data: { data } } = response

    commit('SET_SETTINGS_STUDIOS_DATA', data)
    commit('SET_SETTINGS_STUDIOS_LOADING', false)
    commit('SET_SETTINGS_STUDIOS_ERROR', false)
  }, (response) => {
    commit('SET_SETTINGS_STUDIOS_LOADING', false)
    commit('SET_SETTINGS_STUDIOS_ERROR', true)
  })
}
