    ...
    <div class="wrapper lm-m-register-page-wrapper">
      <div class="lm-m-register-wrap-form">
        <template v-if="(isAuthorized === false)">
          ...
          <div class="lm-social">
            <div class="lm-social-apple" @click="_handleClickOAuthApple()">
        ...
        <template v-if="(isAuthorized === true && hasPhone === false)">

          <h2 class="heading heading_h2 mb_3">Вход</h2>
          <div class="m-text_normal color_gray-3 mb_9">Подтвердите номер телефона, чтобы продолжить авторизацию</div>
          ...
          <div>
            <button class="btn btn_primary w_100 mb_6" :disabled="(hasPhoneFiled === false)" @click.stop.prevent="_handleClickAttachPhoneUser">Продолжить</button>
        ...
        <template v-if="(isAuthorized === true && hasPhone === true)">
        ...
        return new Promise((resolve, reject) => {
          ...
          apiPromise.then(response => {
            ...
            this.$store.dispatch('auth/setToken', { accessToken: token, refreshToken: token }).then(result => {
              this.$store.dispatch('auth/getUserByAPI').then( user => {
                const { isRegistered, hasPhone } = user
                if (hasPhone) {
                  if (isRegistered) {
                    this._handleRedirectAfterAuth().then(() => { Indicator.close() }).catch(() => { Indicator.close() } )
                  } else this.$router.replace({ name: 'user.register' }, () => { Indicator.close() }, () => { Indicator.close() })
