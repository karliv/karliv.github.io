          ...
          <div class="wrapper">
            <ul>
              ...
              <li>
                ...
                <span>{{ $t('mainpage.features.useful-contacts') }}</span>
              </li>
            </ul>
          </div>
        </section>
        <section class="lm-m-section-2">
          <div class="wrapper">
            <img src="/dist/img/main/lm-page-2.svg" alt="Ты талантлив, но тебе не предлагают работу?">
            <h2>{{ $t('mainpage.job.talented') }} <br> {{ $t('mainpage.job.job') }}</h2>
<!--            <div class="desc">{{ $t('mainpage.job.initiative') }} <br> {{ $t('mainpage.job.apply') }} <br> {{ $t('mainpage.job.portfolio') }} <br> {{ $t('mainpage.job.acquaintance') }}-->
            <div class="desc">{{ $t('mainpage.job.initiative') }} <br>{{ $t('mainpage.job.portfolio') }}  <br> {{ $t('mainpage.job.acquaintance') }}
            </div>
            <div class="lm-m-center">
              <router-link :to="{ name: 'register' }" class="lm-m-primary-button" @click.native="_destroyToken({ reload: false })">{{ $t('mainpage.job.get-job') }}</router-link>
            </div>
          </div>
        </section>
        <section class="lm-m-section-3">
          <div class="wrapper">
            <img src="/dist/img/main/lm-li-4.svg" alt="Воплоти свой проект в жизнь">
            <h2>{{ $t('mainpage.project.project') }} <br> {{ $t('mainpage.project.come-true') }}</h2>
            <div class="desc">
              {{ $t('mainpage.project.talent-base') }} <br> Услуги по продвижению <br> Поиск профессионалов
            </div>
            <div class="lm-m-center">
              <router-link :to="{ name: 'register' }" class="lm-m-primary-button red" @click.native="_destroyToken({ reload: false })">{{ $t('mainpage.project.create-project') }}</router-link>
            </div>
          </div>
        </section>
        <section class="lm-m-section-4">
          <div class="wrapper">
            <img src="/dist/img/main/lm-page-5.svg" alt="Хочешь заработать на своём проекте?">
            <h2>{{ $t('mainpage.download.comfort') }}</h2>
            <div class="desc">У тебя наверняка есть телефон, а у нас <br> есть мобильное приложение</div>
            <div class="lm-app-links">
              <div>
                <a target="_blank" href="https://itunes.apple.com/ru/app/linkmuse/id1309529188?mt=8" class="lm-link-app">
                  <img src="/dist/img/main/app-store.svg" alt="App store">
                </a>
              </div>
              <div>
                <a target="_blank" href="https://play.google.com/store/apps/details?id=com.linkmuse&hl=ru" class="lm-link-app">
                  <img src="/dist/img/main/google-play.svg" alt="Google play">
                </a>
              </div>
            </div>
          </div>
