      ...
      <div :class="['modal__content_body', { 'modal__content_body-empty': isEmpty }]" ref="wrapper">
          ...
          <div class="personal-list column">
            <mt-loadmore :top-method="_handleLoadTopUsers" @top-status-change="_handleTopStatusChangeUsers" ref="loadmore">
              <div v-infinite-scroll="_handleInfiniteScroll" infinite-scroll-disabled="infiniteDisabled" infinite-scroll-distance="200" :style="{ minHeight: height }">
                <user-card v-for="(user, key) in users" :key="`following-user-${user.id}-${key}`" :user="user" :isCurrentUser="isCurrentUser"
                           @changeFollow="_handleChangeFollowUser">
                </user-card>
        ...
        <template v-else>
          <div class="page__plug page__plug_feed_empty-list mt_1">
            ...
            <div class="m-text_normal-2">Мы покажем их, как только вы на<br> кого-то подпишитесь</div>
          </div>
        </template>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    async asyncData ({ store, route }) {
      ...
      await store.dispatch('user/getUserFollowingsByUsername', {
        username,
        offset: 0
      })
    },
    ...
    methods: {
      ...
      _handleApplyFilter () {
        this._clearFollowings().then(() => {
          ...
          this._fetchFollowings({ username, fields, offset: 0 }).then(() => {
