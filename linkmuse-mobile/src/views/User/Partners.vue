      ...
      <div :class="['modal__content_body', { 'modal__content_body-empty': isEmpty }]" ref="wrapper">
            ...
            <mt-loadmore :top-method="_handleLoadTopUsers" @top-status-change="_handleTopStatusChangeUsers" ref="loadmore">
              <div v-infinite-scroll="_handleInfiniteScroll" infinite-scroll-disabled="infiniteDisabled" infinite-scroll-distance="200" :style="{ minHeight: height }">
                <user-card v-for="(user, key) in users" :key="`partner-user-${user.id}-${key}`" :user="user" :isCurrentUser="isCurrentUser"
                           @destroyPartner="_handleChangePartnerUser">
                </user-card>
        ...
        <template v-else>
          <div class="page__plug page__plug_feed_empty-list mt_1">
            ...
            <div class="m-text_normal-2">Мы покажем их, как только вы<br> кого-нибудь добавите</div>
          </div>
        </template>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    async asyncData ({ store, route }) {
      ...
      await store.dispatch('user/getUserPartnersByUsername', {
        username,
        offset: 0
      })
    },
    ...
    methods: {
      ...
      _handleApplyFilter () {
        this._clearPartners().then(() => {
          ...
          this._fetchPartners({ username, fields, offset: 0 }).then(() => {
