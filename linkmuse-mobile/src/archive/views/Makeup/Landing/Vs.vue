<template>
  <div class="lm-vs-landing-talent">
    <div class="lm-vs-landing-talent-header vs">
      <div class="lm-vs-landing-talent-header-wrap">
        <div class="title">Зарабатывай больше с Linkmuse</div>
        <div class="desc">Самая актуальная проблема при отборе кандидата — отсутствие профессиональных фотографий, соответствующих требованиям кастинг-директоров и режиссеров
          <br><br>
          Сотрудничество с фотографами решит вопрос
          подготовки качественного портфолио
        </div>
        <a href="javascript:void(0)" style="min-width: 196px" class="lm-white-button middle">Подать заявку</a>
      </div>
    </div>
    <div class="lm-vs-landing-talent-line first">
      <div class="title">Выберите подходящие <br> направления:</div>
      <div class="lm-vs-landing-talent-getting-wrap">
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M44 2H4a2 2 0 0 0-2 2v40a2 2 0 0 0 2 2h40a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2zM5 6a1 1 0 0 1 1-1h6a1 1 0 0 1 0 2H7v5a1 1 0 0 1-2 0V6zm7 37H6a1 1 0 0 1-1-1v-6a1 1 0 1 1 2 0v5h5a1 1 0 1 1 0 2zm13-8a1 1 0 1 1-2 0v-4.08A7.005 7.005 0 0 1 17.08 25H13a1 1 0 0 1 0-2h4.08A7.004 7.004 0 0 1 23 17.08V13a1 1 0 0 1 2 0v4.08A7.004 7.004 0 0 1 30.92 23H35a1 1 0 0 1 0 2h-4.08A7.005 7.005 0 0 1 25 30.92V35zm18 7a1 1 0 0 1-1 1h-6a1 1 0 1 1 0-2h5v-5a1 1 0 1 1 2 0v6zm0-30a1 1 0 0 1-2 0V7h-5a1 1 0 0 1 0-2h6a1 1 0 0 1 1 1v6z"/>
          </svg>
          <span>Съёмка фото</span>
        </div>
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M42.487 1.817a.997.997 0 0 0-.424-.646c-.223-.15-.498-.205-.759-.15l-5.239 1.07-4.193 8.857L42.77 8.724a1 1 0 0 0 .783-1.162l-1.065-5.745zM12.275 14.947l7.348-1.5 4.193-8.857-7.348 1.5-4.193 8.857zM33.614 2.59l-7.348 1.5-4.193 8.857 7.348-1.5 4.193-8.857zM14.018 6.59L2.697 8.901a1 1 0 0 0-.78 1.18l1.168 5.722a1 1 0 0 0 1.18.779l5.56-1.135 4.193-8.857zM44 19H4a1 1 0 0 0-1 1v26a1 1 0 0 0 1 1h40a1 1 0 0 0 1-1V20a1 1 0 0 0-1-1zM30.537 33.844l-11 7a1.007 1.007 0 0 1-1.018.033A1.001 1.001 0 0 1 18 40V26a1.001 1.001 0 0 1 1.538-.844l11 7a1.003 1.003 0 0 1-.001 1.688z"/>
          </svg>
          <span>Съемка и монтаж
видеовизитки</span>
        </div>
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M46 1a1 1 0 0 0-1 1v5h-6V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1 1v5H3V2a1 1 0 1 0-2 0v44a1 1 0 1 0 2 0v-5h6v5a1 1 0 0 0 1 1h28a1 1 0 0 0 1-1v-5h6v5a1 1 0 1 0 2 0V2a1 1 0 0 0-1-1zM9 39H3v-6h6v6zm0-8H3v-6h6v6zm0-8H3v-6h6v6zm0-8H3V9h6v6zm28 10H11v-2h26v2zm8 14h-6v-6h6v6zm0-8h-6v-6h6v6zm0-8h-6v-6h6v6zm0-8h-6V9h6v6z"/>
          </svg>
          <span>Монтаж шоурилла</span>
        </div>
      </div>
      <a href="javascript:void(0)" style="min-width: 196px" class="lm-primary-button middle">Подать заявку</a>
    </div>
    <div class="lm-vs-landing-talent-line cooperation">
      <div class="title">При сотрудничестве <br> вы получите:</div>
      <div class="lm-vs-landing-cooperation-wrap">
        <div class="lm-vs-landing-cooperation-item">
          <span>Дополнительный источник дохода</span>
        </div>
        <div class="lm-vs-landing-cooperation-item">
          <span>Гарантированные онлайн-выплаты</span>
        </div>
        <div class="lm-vs-landing-cooperation-item">
          <span>Индивидуальный график</span>
        </div>
        <div class="lm-vs-landing-cooperation-item">
          <span>Техническую поддержку</span>
        </div>
      </div>
      <a href="javascript:void(0)" style="min-width: 196px" class="lm-primary-button middle">Подать заявку</a>
    </div>
    <div class="lm-vs-landing-talent-line first no-mt">
      <div class="title">Вы нам подходите, <br> если у вас есть:</div>
      <div class="lm-vs-landing-talent-getting-wrap">
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M39.969 16.969a.5.5 0 0 0-.5.5v4.625a.5.5 0 0 1-.5.5h-1.813a.5.5 0 0 1-.5-.5V.5a.5.5 0 0 0-.5-.5h-1.812a.5.5 0 0 0-.5.5v1.813a.5.5 0 0 1-.5.5H14.656a.5.5 0 0 1-.5-.5V.5a.5.5 0 0 0-.5-.5h-1.812a.5.5 0 0 0-.5.5v21.594a.5.5 0 0 1-.5.5H9.03a.5.5 0 0 1-.5-.5v-4.625a.5.5 0 0 0-.5-.5H6.22a.5.5 0 0 0-.5.5V30.53a.5.5 0 0 0 .5.5h5.897a.5.5 0 0 1 .3.1l8.72 6.541a.5.5 0 0 1 .01.793l-8.856 6.984a.5.5 0 0 0-.09.692l1.086 1.45a.5.5 0 0 0 .71.093l9.695-7.621a.5.5 0 0 1 .618 0l9.696 7.62a.5.5 0 0 0 .709-.092l1.086-1.45a.5.5 0 0 0-.09-.692l-8.857-6.984a.5.5 0 0 1 .01-.793l8.721-6.54a.5.5 0 0 1 .3-.1h5.897c.276 0 .5-.232.5-.508V17.476a.507.507 0 0 0-.5-.507H39.97zM33.344 5.625a.5.5 0 0 1 .5.5v7.531a.5.5 0 0 1-.5.5H14.656a.5.5 0 0 1-.5-.5V6.125a.5.5 0 0 1 .5-.5h18.688zM14.156 17.469a.5.5 0 0 1 .5-.5h18.688a.5.5 0 0 1 .5.5v4.625a.5.5 0 0 1-.5.5H14.656a.5.5 0 0 1-.5-.5v-4.625zM24.3 36.08a.5.5 0 0 1-.6 0l-5.532-4.149a.5.5 0 0 1 .3-.9h11.064a.5.5 0 0 1 .3.9l-5.532 4.15z"/>
          </svg>
          <span>Своя либо снятая в аренду студия</span>
        </div>
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M20.655 24.74c5.77 0 10.448-4.791 10.448-10.703 0-5.911-4.678-10.704-10.448-10.704-5.771 0-10.45 4.793-10.45 10.704 0 5.911 4.679 10.704 10.45 10.704zm3.719-6.48c.668 0 1.123.652.679 1.15-.893 1.003-2.482 1.67-4.293 1.67-1.81 0-3.4-.667-4.292-1.67-.445-.498.01-1.15.678-1.15h7.228zm20.368 18.08l-9.56 8.161a.723.723 0 0 1-.971-.045l-4.76-4.877a.753.753 0 0 1 0-1.044l1.02-1.043a.71.71 0 0 1 1.018 0l2.711 2.776a1 1 0 0 0 1.365.062l7.332-6.258a.71.71 0 0 1 1.014.094l.923 1.134a.753.753 0 0 1-.093 1.04zM28.33 40.727l.44.45c.57.585.235 1.564-.578 1.65-2.11.224-4.603.367-7.537.367-10.075 0-15.005-1.686-16.813-2.51-.466-.213-.842-.82-.842-1.343v-3.529c0-5.287 3.93-9.673 8.944-10.26a.82.82 0 0 1 .502.14c2.312 1.715 5.143 2.739 8.209 2.739s5.896-1.024 8.208-2.738a.82.82 0 0 1 .503-.141c5.014.587 8.943 4.973 8.943 10.26a.13.13 0 0 1-.045.099l-2.602 2.221a1 1 0 0 1-1.365-.062l-1.687-1.728a2.264 2.264 0 0 0-3.26 0l-1.02 1.044c-.9.922-.9 2.42 0 3.34z"/>
            <path fill="#0063DA" d="M15.833 19.024a1 1 0 0 1 1-1h8.5a1 1 0 0 1 1 1v1.586a1 1 0 0 1-1 1h-8.5a1 1 0 0 1-1-1v-1.586z"/>
          </svg>
          <span>Статус ИП или ЮР лица</span>
        </div>
        <div class="lm-vs-landing-talent-getting-item">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none" viewBox="0 0 48 48">
            <path fill="#0063DA" d="M25.406 0C24.63 0 24 .629 24 1.406V5.72a1.405 1.405 0 1 0 2.813 0V1.406C26.813.63 26.183 0 25.405 0zM37.65 3.318c-.549-.55-1.439-.55-1.988 0l-2.813 2.813a1.406 1.406 0 1 0 1.989 1.988l2.813-2.812c.549-.55.549-1.44 0-1.989zM45.094 19.781H42.28a1.405 1.405 0 1 0 0 2.813h2.813a1.405 1.405 0 1 0 0-2.813zM5.719 19.781H2.906a1.405 1.405 0 1 0 0 2.813H5.72a1.405 1.405 0 1 0 0-2.813zM17.963 6.13L15.15 3.319a1.406 1.406 0 1 0-1.988 1.989l2.813 2.812a1.406 1.406 0 1 0 1.988-1.988zM9.16 10.086L6.348 8.68a1.407 1.407 0 0 0-1.258 2.515l2.812 1.407a1.406 1.406 0 1 0 1.258-2.516zM43.54 9.309a1.408 1.408 0 0 0-1.888-.63l-2.812 1.407a1.406 1.406 0 1 0 1.258 2.516l2.812-1.407a1.406 1.406 0 0 0 .63-1.886zM38.063 29.625V16.969a2.813 2.813 0 1 0-5.626 0v-2.813a2.812 2.812 0 0 0-5.624 0v-2.812a2.812 2.812 0 1 0-5.625 0v2.812a2.812 2.812 0 0 0-5.625 0v2.955c.463-.067.924-.142 1.406-.142H24a5.63 5.63 0 0 1 5.625 5.625c0 2.518-1.674 4.63-3.96 5.344.709 1.354 1.148 2.868 1.148 4.5a1.406 1.406 0 1 1-2.813 0 7.04 7.04 0 0 0-7.031-7.032H24a2.813 2.813 0 0 0 0-5.625h-7.031a7.04 7.04 0 0 0-7.032 7.032v2.812a9.863 9.863 0 0 0 5.626 8.896v3.854a2.813 2.813 0 0 0-2.813 2.813v1.406c0 .776.63 1.406 1.406 1.406h19.688c.776 0 1.406-.63 1.406-1.406v-1.407a2.813 2.813 0 0 0-2.813-2.812v-3.854a9.863 9.863 0 0 0 5.626-8.896z"/>
          </svg>
          <span>Профессионализм и мотивация</span>
        </div>
      </div>
      <a href="javascript:void(0)" style="min-width: 236px" class="lm-primary-button middle">начать зарабатывать</a>
    </div>
    <div class="lm-vs-landing-talent-line">
      <div class="title">Пакеты:</div>
      <div class="lm-vs-landing-talent-service-wrap">
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/actor.png" alt="">
          <div class="item-title">Портфолио актера</div>
          <div class="desc">Не менее 3 тест-фото и
            видеовизитка до 1 мин</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">2500 ₽</span>
          </div>
        </div>
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/model.png" alt="">
          <div class="item-title">Модельное портфолио</div>
          <div class="desc">Не менее 5 тест-фото, видеовизитка до 1,5 мин и замер параметров</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">3500 ₽</span>
          </div>
        </div>
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/child.png" alt="">
          <div class="item-title">Портфолио для детей</div>
          <div class="desc">Не менее 7 тест-фото и
            видеовизитка до 1 мин</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">4500 ₽</span>
          </div>
        </div>
      </div>
    </div>
    <div class="lm-vs-landing-talent-line last">
      <div class="title">Отдельные услуги:</div>
      <div class="lm-vs-landing-talent-service-wrap">
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/showrill.png" alt="">
          <div class="item-title">Шоурил</div>
          <div class="desc">До 3 минут</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">4500 ₽</span>
          </div>
        </div>
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/photo.png" alt="">
          <div class="item-title">Фотографии</div>
          <div class="desc">Не менее 5 тестовых фото</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">5500 ₽</span>
          </div>
        </div>
        <div class="lm-vs-landing-talent-service-item">
          <img src="/dist/img/vs/landing/video.png" alt="">
          <div class="item-title">Видеовизитка</div>
          <div class="desc">До 1,5 минут</div>
          <div class="item-footer">
            <a href="javascript:void(0)" class="lm-primary-button middle">Заказать</a>
            <span class="price">6500 ₽</span>
          </div>
        </div>
      </div>
    </div>
    <footer class="footer-nav">
      <div class="wrapper">
        <div class="lm-flex-bettwen menu">
          <nav>
            <div class="title">Страницы</div>
            <ul>
              <li>
                <router-link :to="{ name: 'talents.index' }">Таланты</router-link>
              </li>
              <li>
                <router-link :to="{ name: 'castings.index' }">Кастинги</router-link>
              </li>
              <li>
                <router-link :to="{ name: 'articles.index' }">Блог</router-link>
              </li>
            </ul>
          </nav>
          <nav>
            <div class="title">Ссылки</div>
            <ul>
              <li>
                <a target="_blank" href="https://play.google.com/store/apps/details?id=com.linkmuse&hl=ru">Скачать для Android</a>
              </li>
              <li>
                <a target="_blank" href="https://itunes.apple.com/ru/app/linkmuse/id1309529188?mt=8">Скачать для IOS</a>
              </li>
              <li>
                <router-link :to="{ name: 'register' }">Регистрация</router-link>
              </li>
              <li>
                <router-link :to="{ name: 'login' }">Вход</router-link>
              </li>
            </ul>
          </nav>
          <nav>
            <div class="title">Продукт</div>
            <ul>
              <li>
                <router-link :to="{ name: 'index' }">О продукте</router-link>
              </li>
              <li>
                <router-link :to="{ name: 'interviews.index' }">BIO</router-link>
              </li>
            </ul>
          </nav>
          <nav>
            <div class="title">Контакты</div>
            <ul>
              <li>
                <a href="mailto:help@linkmuse.com">Написать по почте</a>
              </li>
            </ul>
          </nav>
        </div>
        <div class="lm-flex-bettwen social">
          <div class="social-wrap">
            <ul>
              <li class="vk"><a href="https://vk.com/link_muse" target="_blank"></a></li>
              <li class="fb"><a href="https://www.facebook.com/linkmuseofficial" target="_blank"></a></li>
              <li class="instagram"><a href="https://www.instagram.com/linkmuse/" target="_blank"></a></li>
              <li class="youtube" style="display: none;"><a href="javascript:void(0)"></a></li>
            </ul>
          </div>
          <div class="desc">
            <span>Сообщество творческих профессионалов</span>
            <span>Linkmuse © {{ $moment() | moment('Y') }}</span>
          </div>
        </div>
      </div>
    </footer>
  </div>

</template>

<script>
  export default {
    metaInfo: {
      title: 'Лендинг для ВС'
    },
    computed: {}
  }
</script>
