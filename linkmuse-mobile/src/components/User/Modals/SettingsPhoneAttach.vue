<template>
  <mt-popup :class="['lm-m-modal-wrap', 'lm-modal-attach-phone']" v-model="value" position="center" :modal="true" :lockScroll="true" @input="_handleInputPopup">
    ...
    <div class="modal-attach-phone-content column ta_center">
      ...
      <template>
        ...
        <template v-else-if="counting === false">
          <a href="javascript:void(0)" class="m-text_bold color_main" @click="_attachPhone">Отправить код ещё раз</a>
        </template>
      </template>
    </div>
  </mt-popup>
...
<script>
  ...
  export default {
    ...
    created () {
      const { phone, token } = this
      this._.merge(this.fields, { phone, token })
    },
    mounted () {
