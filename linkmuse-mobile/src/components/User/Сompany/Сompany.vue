  ...
  <div class="card declaration__card">
    <div class="row mb_3">
      <template v-if="(isCatalogPage === true)">
        <router-link :to="{ name: 'users.companies.show', params: { id } }" class="profile-card__avatar" >
      ...
      <div :class="['column', { 'ml_4': (isCatalogPage === true) }]">
        ...
        <router-link :to="{ name: 'users.companies.show', params: { id } }" class="profile-card__description">
    ...
    <div class="section declaration__section profile-card__buttons justify_evenly align_center">
      <template v-if="isCurrentUser">
        <template v-if="(isPublished === true)">
          <router-link :to="{ name: 'users.companies.edit', params: { id } }" class="btn">Редактировать</router-link>
        </template>
        <template v-if="(isPublished === false)">
          <a href="javascript:void(0)" class="btn" @click="_handleClickPublishCompany">Опубликовать</a>
        </template>
      </template>
      <template v-else>
        <template v-if="phone">
