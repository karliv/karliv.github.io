  ...
  export default {
    ...
    methods: {
      ...
      _handleClickSendButton () {
        let { fields: { state, message, attachments }, folderId } = this.modal
        ...
        this.$api.post(`api/v1/folders/${folderId}/newsletter`, { state, message, attachments }).then(response => {
          let { data } = response
