<template>
  <div :class="[ { 'drag' : drag, 'empty' : empty, 'disabled': disabled  }, 'lm-dropzone' ]" :style="{ 'margin-bottom': errorsHeight }">
    <label :for="inputId" class="lm-dropzone--uploader">
      <template v-if="empty">
        <template v-if="!dictDefaultMessage">
          ...
          <template v-if="label">
            <div class="lm-dropzone--uploader__desc">
              <div class="mb_2">
                {{ label }}
              </div>

              <template v-if="multiple && maxItems">
                <span>Максимальное кол-во<br> файлов {{ maxItems }}</span>
              </template>
            </div>
          </template>
        </template>
    ...
    <div class="lm-dropzone--errors" v-if="hasErrors" :style="[{ visibility: parseInt(errorsHeight) }]" :ref="'errors'">
    ...
    methods: {
      ...
      _handleChange (el) {
        ...
        this._.forEach(files, (file, i) => {
          if (this.maxItems && this.multiple && count === this.maxItems) {
            Toast({
