<template>
  <mt-popup class="lm-m-modal-wrap modal__content modal__content_declaration modal__content_declaration_casting" v-model="modal.value" position="center" :modal="true" @input="_handleInputPopup">
    <svg class="icon icon_info modal__icon_declaration mt_2">
      <use xlink:href="/dist/img/icons/_declaration/sprite.svg#icon_info" />
    </svg>
    <div class="modal__title modal__title_declaration_casting">Решили откликнуться?</div>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$root.$on('open-modal-access-response', (url) => {
        this.modal.url = url
