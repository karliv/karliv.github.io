<template>
    <div>
      <div class="lm-m-dialog-attaches" v-if="attachments.items.length">
        <template v-for="item in attachments.items">
          <template v-if="!item.remove">
            <div :class="['lm-m-dialog-attach', item.type, 'attachments--item-wrap', { 'loading': item.loading, 'show-item': item.show, 'hide-item': item.hide, 'error': item.error }]">
              <template v-if="item.loading || item.error">
                <a href="javascript:void(0)" class="attachments--item-media">
                  <template v-if="item.type === 'photo' || item.type === 'video'">
                    ...
                    <template v-if="item.type === 'video'"><i class="lm-play-icon"></i></template>
                  </template>

                  <template v-if="item.type === 'document'">
                    <div class="document">
                      <div class="lm-m-attach-document-name">{{ item.extension.toUpperCase() }}</div>
                    </div>
                  </template>
              ...
              <template v-if="!item.loading && !item.error">
                ...
                <template v-if="item.type === 'video'">
                  <media :media="item" type="video" class="attachments--item-media"
                    ...
                    <img :src="item.thumbnail" :alt="item.description" :ref="item.uid">
                    <i class="lm-play-icon"></i>
                  </media>
                </template>
                <template v-if="item.type === 'document'">
                  <div class="document">
                    <a :href="item.path" download class="lm-m-attach-document-name">{{ item.extension.toUpperCase() }}</a>
                  </div>
                </template>
              </template>

              <i class="lm-close" @click.stop.prevent="_handleClickRemoveAttachmentItem(item.uid)"></i>
            </div>
          </template>
        </template>
      </div>

      <div class="lm-m-dialog-write">
        <div class="lm-input-wrap">
          <emoji-input-area name="message"
              id="emoji-area-dialog"
              :placeholder="$t('messages.open-dialogue.start')"
              :readonly="false"
        ...
        <div class="lm-m-dialog-show-actions">
          <div class="dialog-attach">
            <label for="lm-upload-files-messages" :class="[ 'lm-attach-icon', { 'active': (uploadingAttachmentsCount), disabled: (canUploadAttachments === false) }]"></label>
          ...
          <div class="dialog-emoji" v-if="(true === false)">
            <i class="lm-smile-icon active"></i>
          </div>
          <div class="dialog-send" @click="_handleClickSendButton">
            <i class="lm-send-icon"></i>
          </div>
        </div>
...
<script>
  ...
  import { generateUUID, isBrowser, humanFileSize, resizeImage, has } from '@utils'
    ...
    methods: {
      ...
      _handleUploadAttachmentError (uid, text) {
        ...
        this.$nextTick(() => {
          ...
          setTimeout(() => this._handleUploadRemoveAttachment(uid), 6000)
        })
      },
