<template>
  <div :class="['card', 'declaration__card', { 'card_premium': (isCatalogPage === true && isFasten === true), 'opacity_64': (isArchived === true) } ]">
    ...
    <template v-if="( (casting.name && casting.name.length) || (casting.roles && casting.roles.length) )">
      <router-link :to="{ name: 'board.castings.show', params: { slug, id } }" class="section declaration__section">
        <template v-if="( casting.name && casting.name.length )">
          <div class="heading_h5 declaration__heading">
        ...
        <div class="declaration__list declaration__list_parameters">
          <template v-if="( roles && roles.length )">
            <div>{{ roles.slice(0, 2).join(', ') }} <template v-if="(roles.length > 2)">...</template></div>
          </template>
        ...
        <div class="row row_justify_center">
          <template v-if="(isCurrentUser === false)">
            <template v-if="(isAuthorized === true && isResponded === false && forTrusted === true && canRespond === false)">
              <div class="d-column d-card__disabled_respond w_100">
                <div class="heading heading_h5">Отклик недоступен</div>
                <template v-if="(isExecutor === true)">
                  <template v-if="((portfolioBadQuality === null || hasPortfolio === false) && hasCertification === false)">
                    <div class="d-card__disabled_respond__description">
                      <span>Для отклика необходимо портфолио.</span>
                      <span>Пройдите верификацию, чтобы получить профессиональные фото и статус проверенного исполнителя.</span>
                      <router-link :to="{ name: 'certifying.executor.index' }" class="d-link mt_1">Узнать подробнее</router-link>
                    </div>
                    <div class="d-card__disabled_respond__description mt_3">
                      <span>Или обновите свой профиль самостоятельно</span>
                      <router-link :to="{ name: 'user.show', params: { username: user.username } }" class="d-link mt_1">Обновить</router-link>
                    </div>
                  </template>
                  <template v-if="(portfolioBadQuality === true && hasCertification === false)">
                    <div class="d-card__disabled_respond__description">
                      <span>Модератор оценил ваше портфолио как недостаточно качественное.</span>
                      <span>Для выяснения причин посмотрите личные сообщения.</span>
                      <router-link :to="{ name: 'messages.index' }" class="d-link mt_1">Перейти в личные сообщения</router-link>
                    </div>
                  </template>
                </template>
                <template v-else-if="(isCustomer === true)">
                  <div class="d-card__disabled_respond__description">
                    <span>Для отклика необходим аккаунт таланта</span>
                    <template v-if="(hasSecondaryUser === true)">
                      <a href="javascript:void(0)" @click="_handleClickCreateAccount()" class="d-link mt_1">Войти как талант</a>
                    </template>
                    <template v-else-if="(hasSecondaryUser === false)">
                      <a href="javascript:void(0)" @click="_handleClickCreateAccount(1)" class="d-link mt_1">Добавить аккаунт</a>
                    </template>
                  </div>
                </template>
              </div>
            ...
            <template v-else>
              <template v-if="(isAuthorized === true && canRespondOnCasting === false)">
                <button :class="['btn']" :disabled="true">Отклики ограничены</button>
              </template>
              <template v-else>
          ...
          <template v-if="(isCurrentUser === true && (isCatalogPage === true || (isCatalogPage === false && (isPublished === true || isArchived === true))))">
            <router-link :to="{ name: 'board.castings.show', params: { slug, id }, query: { responses: true } }" :class="['btn', { 'btn_secondary': (isArchived === true) }]">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      roles () {
        return (this.casting.roles && this.casting.roles.length) ? this.casting.roles.map((role => role.text)) : []
      },
      genders () {
        ...
        if (this.isAuthorized === true) {
          if (this.canRespond) {
            if (this.isResponded === false) {
              let message = (this.city && this.user && this.profile && has(this.profile, 'city.data.id') && this.profile.city.data.id !== this.city.id)
