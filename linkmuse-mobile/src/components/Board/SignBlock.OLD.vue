<template>
  <div :class="['section', 'form__section form__section_declaration', { 'form__section_declaration_disabled': (disabled) }]">
    ...
    <template v-if="(isAuthorized === true && isRegistered === true && hasPhone === false)">
      <div class="form__group form__group_declaration">
        <label :for="KEYS_ATTACH['phone']" class="label input__label">
          Телефон
        </label>

        <input type="text" autocomplete="off" autocapitalize=off v-mask-phone
               :name="normalizeDotsToArrow(KEYS_ATTACH['phone'])"
               :id="KEYS_ATTACH['phone']"
               v-validate="'required|phone'"
               data-vv-name="phone"
               placeholder="+7 900 000 00 00"
               :data-vv-as="'Телефон'"
               data-vv-scope="attached"
               v-model="attached.fields[KEYS_ATTACH['phone']]"
               :disabled="(disabled)"
               @keyup.enter="_handleClickAttachPhoneUser"
               :class="[{ 'error': (verrors.has('attached.' + normalizeDotsToArrow(KEYS_ATTACH['phone']))) }, 'input', 'form__input', 'form__input_declaration']">
        <template v-if="(verrors.has('attached.' + normalizeDotsToArrow(KEYS_ATTACH['phone'])))">
          <div class="form__notification form__notification_declaration form__notification_declaration_required">{{ verrors.first('attached.' + normalizeDotsToArrow(KEYS_ATTACH['phone'])) }}</div>
        </template>
      </div>

      <div class="form__group form__group_declaration">
        <button class="btn btn_primary" :disabled="(disabled)" @click="_handleClickAttachPhoneUser">Продолжить</button>
      </div>
    </template>
  </div>
</template>
