...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_vue_loader_lib_selector_type_script_index_0_Create_vue__ = __webpack_require__(1635);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__modules_Board_Location_extends_Form_vue__ = __webpack_require__(1569);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__modules_Board_Location_mixins_form_js__ = __webpack_require__(1573);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__modules_Rating_Enums_events__ = __webpack_require__(438);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__modules_Board_Location_Enums_LocationTransition__ = __webpack_require__(1541);
...
/* harmony default export */ __webpack_exports__["a"] = ({
  extends: __WEBPACK_IMPORTED_MODULE_3__modules_Board_Location_extends_Form_vue__["a" /* default */],
  mixins: [__WEBPACK_IMPORTED_MODULE_2__modules_Board_mixins_form_js__["a" /* default */], __WEBPACK_IMPORTED_MODULE_4__modules_Board_Location_mixins_form_js__["a" /* default */]],
  metaInfo: {
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.next = 2;
              return Promise.all([store.dispatch('board/location/getSources'), store.dispatch('board/location/clearLocationFields')]);
  ...
  mounted: function mounted() {
    ...
    if (this.isAuthorized && this.user) {
      ...
      setTimeout(function () {
        if (Object(__WEBPACK_IMPORTED_MODULE_0__utils__["l" /* has */])(_this2.user, 'contacts.phone') && _this2.user.contacts.phone) {
          _this2.fields.phone = _this2.user.contacts.phone;
        }
      }, 2000);
    }

    var fields = this.$localStorage.get('board.create.location', null, Object);
  ...
  methods: {
    _handleClickActionLocation: function _handleClickActionLocation(transition) {
      ...
      var event = transition === __WEBPACK_IMPORTED_MODULE_6__modules_Board_Location_Enums_LocationTransition__["b" /* PUBLISH */] ? 'location_publication' : 'location_draft';
      this.$gtm.trackEvent({ event: event });
    },
    _handleClickCreateLocation: function _handleClickCreateLocation(transition) {
      var _this3 = this;

      this.fields.transition = transition;
      this.$nextTick(function () {
        _this3._handleValidateFields().then(function () {
          _this3._createLocation().then(function () {
            var username = _this3.user.username;
            var transition = _this3.fields.transition;

            _this3.$localStorage.remove('board.create.location');
            _this3.$store.dispatch('board/location/setHasNewLocationsData', true);

            if (transition === __WEBPACK_IMPORTED_MODULE_6__modules_Board_Location_Enums_LocationTransition__["b" /* PUBLISH */]) {
              if (Object(__WEBPACK_IMPORTED_MODULE_0__utils__["l" /* has */])(_this3.profile, 'without_moderate.castings') && _this3.profile['without_moderate']['castings']) {
                Object(__WEBPACK_IMPORTED_MODULE_1_mint_ui__["Toast"])({ message: 'Объявление успешно опубликовано', iconClass: 'mintui mintui-success' });
                _this3.$router.push({ name: 'board.locations.index' });
              } else {
                var openSuccessCreateBoardModal = _this3.$localStorage.get('success.board.create', false, Boolean);

                if (openSuccessCreateBoardModal === false) {
                  _this3.$modalRouter.push({ name: 'success.board.create' });
                } else Object(__WEBPACK_IMPORTED_MODULE_1_mint_ui__["Toast"])({ message: 'Объявление отправлено на модерацию.', iconClass: 'mintui mintui-success' });
                _this3.$router.push({ name: 'board.locations.user', params: { username: username } });
              }

              _this3.$nsWebViewInterface.emit('rating', __WEBPACK_IMPORTED_MODULE_5__modules_Rating_Enums_events__["b" /* createLocation */]);
            } else {
              Object(__WEBPACK_IMPORTED_MODULE_1_mint_ui__["Toast"])({ message: 'Объявление сохранено в черновик.', iconClass: 'mintui mintui-success' });
              _this3.$router.push({ name: 'board.locations.user', params: { username: username } });
            }
          }).catch(function () {
    ...
    _scrollToErrorFields: function _scrollToErrorFields() {
      var el = document.getElementsByClassName('error');
      if (el && el.length) this.$scrollTo(el[0], 500, { offset: -40 });
      Object(__WEBPACK_IMPORTED_MODULE_1_mint_ui__["Toast"])({ message: 'Проверьте правильность ввода.', iconClass: 'mintui mintui-field-error' });
    },
    _createLocation: function _createLocation() {
      var _this4 = this;
      ...
      return new Promise(function (resolve, reject) {
        ...
        var data = _this4._.cloneDeep(_this4.fields);
        if (_this4.isRoom === false) {
          _this4.$delete(data, 'roomType');
          _this4.$delete(data, 'roomHeight');
          _this4.$delete(data, 'exterior');
        }

        _this4.$api.post('api/v2/boards/locations', data).then(function (response) {
    ...
    _handleClickClearForm: function _handleClickClearForm() {
      ...
      this._clearForm().then(function () {
        _this5.$gtm.trackEvent({ event: 'clear_form' });
    ...
    _clearForm: function _clearForm() {
      ...
      return new Promise(function (resolve, reject) {
        try {
          _this6.uploads = [];
          _this6.showFieldForm = false;
          _this6.$nextTick(function () {
            return _this6.showFieldForm = true;
          });
          _this6.fields = _this6._.cloneDeep(_this6.defaultFields);
          resolve && resolve();
        } catch (e) {
          reject && reject(e);
        }
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(4)("3e025e7e", content, false, {});
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
...
//# sourceMappingURL=76.build.7b4c03c2abb2fb294164.js.map